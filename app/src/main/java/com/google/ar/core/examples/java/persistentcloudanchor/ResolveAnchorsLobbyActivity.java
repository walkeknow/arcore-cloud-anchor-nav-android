...
public class ResolveAnchorsLobbyActivity extends AppCompatActivity {
  ...
  static Intent newIntent(Context packageContext) {
    return newIntent(packageContext, false);
  }

  static Intent newIntent(Context packageContext, boolean drawLines) {
    Intent intent = new Intent(packageContext, ResolveAnchorsLobbyActivity.class);
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    drawLines = getIntent().getBooleanExtra(EXTRA_DRAW_LINES, false);
    MaterialButton resolveButton = findViewById(R.id.resolve_button);
    resolveButton.setOnClickListener((view) -> onResolveButtonPress());
    MaterialButton drawLinesButton = findViewById(R.id.draw_lines_button);
  ...
  private void onResolveButtonPress() {
    ...
    Intent intent = CloudAnchorActivity.newResolvingIntent(this, anchorsToResolve, false);
    startActivity(intent);
  }

  /** Callback function invoked when the Draw Lines Button is pressed. */
  private void onDrawLinesButtonPress() {
    ArrayList<String> anchorsToResolve = new ArrayList<>();
    for (AnchorItem anchorItem : selectedAnchors) {
      if (anchorItem.isSelected()) {
        anchorsToResolve.add(anchorItem.getAnchorId());
    ...
    Intent intent = CloudAnchorActivity.newResolvingIntent(this, anchorsToResolve, true);
    startActivity(intent);
  }
}
